...
jobHandler:Button("Dealer Job", "Pressing this enables the Autofarm for the dealer job.", function()
    ...
    while _G.gamer do
    wait()
    local box = game:GetService("Workspace")["Dealer"][" "].Part.ClickDetector
...
for i,v in pairs(game:GetService("Players").LocalPlayer.PlayerGui.DealerJob:GetDescendants()) do
    if v.ClassName == "TextButton" then
        pcall(function()
            ...
            for x,y in pairs(getconnections(v.MouseButton1Up)) do
                y.Function()
                y:Fire()
                y.Fire()
            end
            for x,y in pairs(getconnections(v.MouseButton1Down)) do
                y.Function()
                y:Fire()
                y.Fire()
            end
    ...
    local homeless1 = workspace.HomelessMan2.ClickDetector
    local homeless2 = workspace.HomelessMan3.ClickDetector
    local homeless3 = workspace.HomelessMan4.ClickDetector
    local homeless4 = workspace.HomelessMan5.ClickDetector
    -- positions
    local homeless5 = workspace.HomelessMan2.Handle
    local homeless6 = workspace.HomelessMan3.Handle
    local homeless7 = workspace.HomelessMan4.Handle
    local homeless8 = workspace.HomelessMan5.Handle
    
    -- 1
    hrp.CFrame = homeless5
    fireclickdetector(homeless1)

    -- 2
    hrp.CFrame = homeless6
    fireclickdetector(homeless2)

    -- 3
    hrp.CFrame = homeless7
    fireclickdetector(homeless3)

    -- 4
    hrp.CFrame = homeless8
    fireclickdetector(homeless5)
end
end)
end
end
...
-- packagejob
jobHandler:Button("Package Delivery", "Pressing this enables the autofarm for the package delivery job.", function()
   ...
   while _G.autofarm do
    wait()
    local pack = game:GetService("Workspace"):FindFirstChild("me").ClickDetector
    local deliver = game:getService("Workspace").Pehoz:FindFirstChild("Part")
    local humanoidrootpart = character.HumanoidRootPart
    local pack1 = game:GetService("Workspace"):FindFirstChild("me").ClickDetector


    fireclickdetector(pack)
    humanoidrootpart.CFrame = deliver.CFrame
   end
end)
